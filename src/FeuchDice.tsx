...
export default function FeuchDice({onBack}:{onBack:()=>void}) {
 ...
 const finishAttack=()=>{if(!rolls||rolling)return;const base=challenges(category(values),values);setOptions(base);setPhase('choose');setOctopusState('loading');setOctopusDetail('Amélioration des trois manigances en cours…');setComment('Marty : « Trois propositions, une amitié en moins. »');const id=++requestId.current;improveDiceChallenges(category(values),values,base,players.map(p=>p.name)).then(flavor=>{if(id!==requestId.current)return;setOptions(base.map((item,i)=>({...item,title:flavor[i]!.title,instruction:item.instruction})));setOctopusState('live');setOctopusDetail('Titres générés par Octopus ; règles locales vérifiables.');}).catch(()=>{if(id===requestId.current){setOctopusState('local');setOctopusDetail('Manigances locales : Octopus indisponible, trop lent ou réponse invalide.');}});};
