...
export default function FeuchDice({onBack}:{onBack:()=>void}) {
 ...
 const [octopusState,setOctopusState]=useState<'loading'|'live'|'local'>('local');
 const requestId=useRef(0);
 useEffect(()=>{let active=true;generateDiceAliases().then(names=>{if(active)setPlayers(prev=>prev.map((p,i)=>({...p,name:names[i]!})));}).catch(()=>{});return()=>{active=false;};},[]);
 const defender=1-attacker;
 const current=players[phase==='defend'||phase==='handoff'?defender:attacker]!;
 const maxRolls=phase==='defend'?challenge?.rolls??2:3;
 const roll=()=>{if(rolls>=maxRolls)return;setValues(prev=>prev.map((v,i)=>held[i]?v:rand()));setRolls(n=>n+1);setComment(phase==='defend'?'Marty : « La dignité est en cours de vérification. »':'Marty : « Le hasard refuse de commenter. »');};
 const resetDice=()=>{setValues([1,2,3]);setHeld([false,false,false]);setRolls(0);};
 const finishAttack=()=>{if(!rolls)return;const base=challenges(category(values),values);setOptions(base);setPhase('choose');setOctopusState('loading');setComment('Marty : « Trois propositions, une amitié en moins. »');const id=++requestId.current;improveDiceChallenges(category(values),values,base,players.map(p=>p.name)).then(flavor=>{if(id!==requestId.current)return;setOptions(base.map((item,i)=>({...item,title:flavor[i]!.title,instruction:item.instruction})));setOctopusState('live');}).catch(()=>{if(id===requestId.current)setOctopusState('local');});};
 ...
 <section className="fd-players">{players.map((p,i)=><div className={'fd-player '+(i===attacker?'active':'')} key={i}><small>{p.name}</small><div className="fd-bricks" aria-label={p.dignity+' points de dignité'}>{[0,1,2].map(n=><i key={n} className={n<p.dignity?'full':'empty'}/>)}</div></div>)}</section>
 ...
 {(phase==='attack'||phase==='defend')&&<div className="fd-controls"><small>LANCER {rolls} / {maxRolls} · TOUCHE UN DÉ POUR LE GARDER</small><div className="fd-dice">{values.map((v,i)=><button key={i} className={held[i]?'held':''} onClick={()=>setHeld(h=>h.map((b,j)=>j===i?!b:b))} aria-label={`Dé ${i+1}, valeur ${v}, ${held[i]?'gardé':'libre'}`}><span>{['','⚀','⚁','⚂','⚃','⚄','⚅'][v]}</span><small>{held[i]?'GARDÉ':'LIBRE'}</small></button>)}</div><button className="fd-main" disabled={rolls>=maxRolls||held.every(Boolean)} onClick={roll}>🎲 LANCER LES DÉS</button><button className="fd-secondary" disabled={!rolls} onClick={phase==='attack'?finishAttack:judge}>{phase==='attack'?'CHOISIR UNE MANIGANCE →':'VALIDER LE DÉFI →'}</button></div>}
 </main>;
}
