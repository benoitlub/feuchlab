...
import {recordLabSession} from './lab-memory';
...
export default function FeuchDice({onBack}:{onBack:()=>void}) {
 ...
 const next=()=>{if(!won&&players[defender]!.dignity===0){recordLabSession('dice',null,null,players[attacker]!.name+' gagne · '+players.map(p=>p.name+': '+p.dignity+' dignité').join(' / '));setPhase('end');return;}setAttacker(defender);setChallenge(null);resetDice();setPhase('attack');setComment('Marty : « À votre tour de gâcher cette belle amitié. »');};
