...
export default function FeuchDice({onBack}:{onBack:()=>void}) {
 ...
 const finishAttack=()=>{if(!rolls)return;setPhase('choose');setComment('Marty : « Trois propositions, une amitié en moins. »');};
 const choose=(c:Challenge)=>{setChallenge(c);resetDice();setPhase('handoff');setComment(`Marty : « ${players[defender]!.name}, votre réputation est convoquée. »`);};
 ...
 const restart=()=>{const shuffled=[...aliases].sort(()=>Math.random()-.5);setPlayers([{name:shuffled[0]!,dignity:3},{name:shuffled[1]!,dignity:3}]);setAttacker(0);setChallenge(null);resetDice();setPhase('attack');setComment('Marty : « Nouveau départ. Même erreur de jugement. »');};
 ...
 {phase==='choose'&&<section className="fd-options">{challenges(category(values),values).map((c,i)=><button key={i} onClick={()=>choose(c)}><small>MANIGANCE 0{i+1}</small><strong>{c.title}</strong><span>{c.instruction}</span><b>CHOISIR ↗</b></button>)}</section>}
