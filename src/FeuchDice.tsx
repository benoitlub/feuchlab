...
export default function FeuchDice({onBack}:{onBack:()=>void}) {
 ...
 useEffect(()=>{let active=true;generateDiceAliases().then(names=>{if(active){setPlayers(prev=>prev.map((p,i)=>({...p,name:names[i]!})));setOctopusState('live');setOctopusDetail('Surnoms générés par Octopus.');}}).catch(()=>{if(active){setOctopusState('local');setOctopusDetail('Surnoms locaux : Octopus indisponible ou réponse invalide.');}});return()=>{active=false;};},[]);
 ...
 const finishAttack=()=>{if(!rolls)return;const base=challenges(category(values),values);setOptions(base);setPhase('choose');setOctopusState('loading');setOctopusDetail('Amélioration des trois manigances en cours…');setComment('Marty : « Trois propositions, une amitié en moins. »');const id=++requestId.current;improveDiceChallenges(category(values),values,base,players.map(p=>p.name)).then(flavor=>{if(id!==requestId.current)return;setOptions(base.map((item,i)=>({...item,title:flavor[i]!.title,instruction:item.instruction})));setOctopusState('live');setOctopusDetail('Titres générés par Octopus ; règles locales vérifiables.');}).catch(()=>{if(id===requestId.current){setOctopusState('local');setOctopusDetail('Manigances locales : Octopus indisponible, trop lent ou réponse invalide.');}});};
